...
class CropGrowthSimulationTest extends TestCase
{
    ...
    protected function captureState(string $timePoint): void
    {
        $this->stageResults[$timePoint] = [
            ...
            'time_since_planting' => $this->formatDiff($this->crop->planted_at, now()),
            'time_in_stage' => $this->formatDiff($this->crop->{$this->crop->current_stage . '_at'}, now()),
            'time_to_next_stage' => $this->crop->timeToNextStage(),
            'expected_harvest' => $this->crop->expectedHarvestDate()->format('Y-m-d H:i:s'),
            'time_to_harvest' => $this->formatDiff(now(), $this->crop->expectedHarvestDate()),
        ];
    }
     ...
     */
    public function testCropGrowthLifecycle(): void
    {
     ...
     */
    public function testDailySnapshots(): void
    {
        echo "\n=== DAILY SNAPSHOTS ===\n";
        ...
        for ($day = 0; $day <= $totalDays; $day++) {
            ...
            if ($day == $this->recipe->germination_days) {
                ...
                $this->crop->refresh();
            } else if ($day == ($this->recipe->germination_days + $this->recipe->blackout_days)) {
